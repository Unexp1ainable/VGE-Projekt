...
def costFunction(v1: BMVert, v2: BMVert, v3: BMVert):
    # min angle
    vec0: Vector
    vec1: Vector
    vec1 = v1.co - v3.co
...
def calculateWeight(vertices: Tuple[BMVert], accessEdge: Tuple[int, int]):
    ...
    if len(vertices) == 3:
        return costFunction(*vertices), -1
    ...
    for _ in range(len(vertices)-2):
        wt = costFunction(vertices[accessEdge[0]], vertices[accessEdge[1]], vertices[selectedVertex])
...
def triangulate(vertices: List[BMVert], accessEdge: Tuple[int, int]) -> List[Tuple[int,int,int]]:
    if len(vertices) < 3:
        return []
    ...
    tmpRes1 = triangulate([vertices[vertex]]+vertices[1:vertex], (0,1))
    tmpRes2 = triangulate([vertices[0]] + vertices[vertex:], (0,1))
